...
public class CrazyOptionals {
    ...
    /**
     * Retrieves an {@link Account} with gmail email using {@link AccountProvider}.
     * If no account is provided or it gets
    ...
    public static Optional<Account> retrieveAccountGmail(AccountProvider accountProvider) {
        return accountProvider.getAccount()
                .filter(x -> x.getEmail().split("@")[1].equals("gmail.com"));
    }
    ...
    /**
     * Finds an {@link Account} with max balance and processes it using
     * {@link AccountService#processAccount(Account)}
     *
     * @param accounts
     * @param accountService
     */
